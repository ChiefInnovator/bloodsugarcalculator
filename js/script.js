document.addEventListener('DOMContentLoaded', function() {
    ...
    const initialConversionDirection = document.querySelector('input[name="conversion-direction"]:checked').value;
    ...
    calculateBtn.addEventListener('click', function() {
        ...
        if (conversionDirection === 'a1c-to-eag') {
            ...
            if (a1cValue < 4.0 || a1cValue > 14.0) {
                showError('Please enter an A1C value between 4.0 and 14.0.');
                return;
            }
            ...
            resultExplanation.textContent = `An A1C of ${a1cValue.toFixed(1)}% corresponds to an estimated average glucose of ${eagMgdl} mg/dL or ${eagMmoll} mmol/L.`;
            resultContainer.classList.remove('hidden');
            
        } else {
            // eAG to A1C conversion
            const eagValue = parseFloat(eagInput.value);
            const eagUnits = document.querySelector('input[name="eag-units"]:checked').value;
            ...
            resultExplanation.textContent = `An estimated average glucose of ${eagValue} ${eagUnitText} corresponds to an A1C of approximately ${a1cValue.toFixed(1)}%.`;
            resultContainer.classList.remove('hidden');
        }
    });
